...
function mountReducer(reducer, initialArg) {
  ...
  const dispatch = dispatchReducerAction.bind(null, currentRenderingFiber, queue);  // 创建dispatch函数，用于后续的action派发
...
export function renderWithHooks(current, workInProgress, Component, props) {
  currentRenderingFiber = workInProgress;  // 设置当前正在渲染的fiber节点
